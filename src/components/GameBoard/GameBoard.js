...
import { getCurrentPlayer, hasWinner } from '../../helpers';
  ...
  return (
    <div className={styles['container']}>
      <div className={styles['game-stats']}>
        <h1>Connect Four</h1>
        {!gameOver && <p className={styles['player']}>Player {player === 'r' ? 'Red' : 'Yellow'}'s Turn</p>}
        {gameOver && <p className={styles['player']}>{player === 'y' ? 'Red' : 'Yellow'} Wins!</p>}
        {gameOver && <button className={styles['reset']}onClick={actions.reset}>Start New Game</button>}
      </div>
